 ...
 */
//http://localhost:8080/api/second-layer02/auto-test
@RestController
@RequestMapping("/api/second-layer02")
public class SecondLayerController02 {
    ...
    public String autoTest() {
        ...
        faceMsg.setSn("FACE_WH_02"); // 数据库中对应的 SN
        faceMsg.setTs(now - 2000);   // 模拟 2 秒前刷脸
        Map<String, Object> facePayload = new HashMap<>();
        facePayload.put("personId", "李四"); // 模拟网关识别出的姓名
        ...
        rfidMsg.setSn("GATE_WH_02"); // 数据库中对应的 SN
        rfidMsg.setTs(now);          // 模拟现在经过闸机
        Map<String, Object> rfidPayload = new HashMap<>();
        rfidPayload.put("tags", java.util.Arrays.asList("涉密载体300", "涉密载体301"));
